...
export default function CalendarWeekView() {
  ...
  const calculateHour = (start: number, end: number) => {
    return `${start * 12 + 2} / span ${(end - start) * 12}`;
  };
  ...
  const calculateWidth = (item: {
    ...
    const occurence = events.filter((event) => event.start === item.start);
    return 100 / occurence.length;
  };
            ...
            <div className="grid flex-auto grid-cols-1 grid-rows-1">
              ...
              >
                {events.map((item, idx) => {
                  return (
                    <li
                      className={`relative mt-px flex sm:col-start-3 z-[${
                        idx + 1
                      }] 
                      hover:z-[99999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999]`}
                      style={{
                        gridRow: `${calculateHour(item.start, item.end)}`,
                      }}
                      key={idx}
                    >
                      <a
                        href="#"
                        className={`group lg:absolute lg:inset-1 flex flex-col overflow-y-auto rounded-lg bg-${item.colore}-50 p-2 text-xs leading-5 hover:bg-${item.colore}-100`}
                      >
                        <p
                          className={`order-1 font-semibold text-${item.colore}-700`}
                        >
                          {item.nome}
                        </p>
                        <p
                          className={`text-${item.colore}-500 group-hover:text-${item.colore}-700`}
                        >
                          <time dateTime="2022-01-12T07:30">
                            {item.start}:00 {item.start > 12 ? "PM" : "AM"}
                          </time>
                        </p>
                      </a>
                    </li>
